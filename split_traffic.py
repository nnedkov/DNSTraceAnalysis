...
def main():
    ...
    user_requests_num = [(user, len(requests)) for user, requests in \
                                               user_requests.iteritems()]
    user_requests_num = sorted(user_requests_num,
                               key=itemgetter(1),
                               reverse=True)
    user_clusters = dict()
    ...
    for user, requests_num in user_requests_num:
        if VERBOSITY_IS_ON:
            print 'User %s is issuing %s request(s)' % (user, requests_num)
    ...
    for i in range(USER_CLUSTERS_NUMBER):
        ...
        sim_input_filepath = '%s/simulation_input_%s.txt' % \
                             (ANALYSIS_RESULTS_DIR, i)

        with open(sim_input_filepath, 'w') as fp:

            for request in sorted(requests):
                fp.write('%s\n' % request)

        print 'File %s has %s requests from %s different users' % \
              (sim_input_filepath,
               len(requests),
               len(users))
